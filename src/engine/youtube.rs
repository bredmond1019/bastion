use anyhow::Result;
use serde_json::json;
use workflow_engine_core::{nodes::AsyncNode, task::TaskContext};
...
impl AsyncNode for McpToolNode {
    ...
    ) -> Result<TaskContext, workflow_engine_core::error::WorkflowError> {
        let transcript_node = task_context
            .nodes
            .get("fetch_transcript")
            .and_then(|n| n.get("transcript"))
            .and_then(|t| t.as_str())
            .unwrap_or("");

        println!(
            "[{}] Sending text to Python MCP server: {}...",
            self.tool_name,
            &transcript_node[..std::cmp::min(20, transcript_node.len())]
        );
...
pub async fn run(url: String) -> Result<()> {
    ...
    let mut context = TaskContext::new("youtube_summary".to_string(), json!({ "url": url }));
