...
- **What:** Drained the follow-up tab left by the 2026-08-01 four-spec arch-review train, as a
  dependency-ordered four-ticket train (waves 272 → 274), each ticket in its own `core/`-sibling
  worktree, each merged locally, then all pushed together as `a598c4d..fb6de7e` (9 commits).
  - `ticket-cargo-bin-path-hardening` (`23058e9`) — `scripts/check-typeshare-drift.sh` +
    `scripts/gen-types.sh` now probe `${CARGO_HOME:-$HOME/.cargo}/bin/typeshare` on a `command -v`
    miss and report *"installed but not on PATH — do NOT reinstall"* instead of a false
    `cargo install` prescription; `docs/setup.md` section added. HQ repo separately got a
    `routine.sh` PATH export + `README.md` (`278fded`, `66ebaed` there). The `~/.zshrc` line was
    **proposed, not applied**.
  - `ticket-epic-weight-dto` (`754cf0f`) — `EpicDto.weight: Option<u8>` copied verbatim in
    `build_epics`; `types/serve.ts` regenerated by real typeshare 1.13.4; **serve-api v0.20 →
    v0.21**. Unblocks bastion-web.
  - `ticket-serve-env-test-race` (`664ec02`) — new `src/testsupport.rs`; authoritative `cargo
    test` gate went from **16/30 failing → 30/30 clean** at `--test-threads=24`, plus a 5/5
    consecutive-run stability proof.
  - `ticket-contract-corpus-uncovered-routes` (`fb6de7e`) — corpus **22 → 32 goldens**, freezing
    `workflows`, `handoff` (both distinct 404 codes), `epics` (post-v0.21, both `weight` branches),
    `blocks/graph` (pinned `max_nodes=50`), and `costs` (DB-free `C006`/`C005` error shapes).
  - Final combined gate on `main`: fmt ✓, clippy `-D warnings` ✓, `cargo test` **1974 passed / 0
    failed** ✓, release ✓, typeshare drift ✓, contract-corpus drift ✓.
- **Why:** The arch-review train shipped v0.17 → v0.20 but left three findings that degraded the
  *authoritative* validation gate itself — a PATH gap that had already cost a full run to a false
  "not installed" diagnosis, an intermittent full-suite flake that `nextest` is structurally blind
  to, and a contract corpus that did not cover the very routes the train had just added. Two
  discoveries went beyond the specs: **(1)** the flake was *two* races, and the named one was not
  the culprit — fixture temp dirs named `{prefix}-{pid}-{nanos}` collided because `process::id()`
  is identical across `cargo test`'s threads and macOS `SystemTime` is microsecond-resolution, so
  same-microsecond tests `remove_dir_all`'d each other's fixtures; **(2)** a latent redaction bug
  from A4 — `BlockGraphDto.root` is canonicalized upstream in `mev` and macOS's `/var →
  /private/var` symlink defeated the redactor's prefix match, freezing a live temp path into a
  golden on first generate. Fixed at the redaction layer per `serve-api.md` §25.5, not silently
  regenerated.
- **Refs:** `planning/arch-review-train-followups.md`; the four `planning/ticket-*/` specs;
  `docs/serve-api.md` §17 (v0.21), §25 (corpus); `planning/handoff.md`.
