...
- [`bastion brain`](../knowledge/brain.md) and [`bastion code`](../knowledge/code.md) — to resolve the corpus / source root
  to scan (both also accept `--root` and `--workspace`, which override it).
- [`bastion momentum`](../boards/momentum.md) — to know which repos to roll up. Unlike the other two, it
...
(`src/serve/notify/`); see [serve-api.md §26](../serve/serve-api.md#26-operator-notification-transport)
...
same chat. See [serve-api.md §26.9](../serve/serve-api.md#269-approve-and-run-resolution-v030-ticket-approve-and-run-seams)
...
[serve-api.md §27](../serve/serve-api.md#27-session-qa-bridge) for
the full contract.
